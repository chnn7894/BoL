...
function ScriptMsg(msg)
  print("<font color=\"#00fa9a\"><b>Simple HTTF Vladimir:</b></font> <font color=\"#FFFFFF\">"..msg.."</font>")
end
...
print("<font color=\"#00fa9a\"><b>Simple HTTF Vladimir:</b> </font><font color=\"#FFFFFF\">Server status: </font><font color=\"#ff0000\"><b>"..Server.."</b></font>")
...
ScriptPath = "/BolHTTF/BoL/master/HTTF/SimpleHttfVladimir.lua".."?rand="..math.random(1,10000)
UpdateURL = "https://"..Host..ScriptPath

VersionPath = "/BolHTTF/BoL/master/HTTF/Version/SimpleHttfVladimir.version".."?rand="..math.random(1,10000)
...
function Variables()
  ...
  R = {delay = 0, radius = 325, range = 625, speed = math.huge, ready, Off = 0, block = false}
  I = {range = 600, ready}
  Z = {range = 1000, ready}
...
function VladimirMenu()
  ...
  Menu = scriptConfig("Simple HTTF Vladimir", "HTTF Vladimir")
...
function Combo()
  ...
  if R.ready and ComboR then
    ...
    if ValidTarget(Target, Q.range) then
      ...
      if Q.ready and ComboQ and QTargetDmg >= Target.health then
        CastQ(Target)
        return
      end
  ...
  if W.ready and ComboW then
    ...
    if not (Q.ready and ComboQ) and not (E.ready and ComboE) then 
...
function KillSteal()
  ...
  if W.ready and KillStealW then
    ...
    if not (Q.ready and KillStealQ) and not (E.ready and KillStealE) then
  ...
  if Menu.Predict.PdOpt == 1 then
    ...
    if Donator then
      ...
      if State == Combo then
        ...
        local Boolean, Pos, Info = Prodiction.GetMinCountCircularAOEPrediction(1, E.range, E.speed, E.delay, E.radius)
        
        if Boolean and Pos and Info.hitchance >= 2 then
          CastE()
        end
